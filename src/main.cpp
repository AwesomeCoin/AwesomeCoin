...
const string strMessageMagic = "AwesomeCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight > 3504000) // no block reward after 5 years
        nSubsidy = 0;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.04 * 24 * 60 * 60; // AwesomeCoin: Roughly once per hour
static const int64 nTargetSpacing = 45; // AwesomeCoin: 45 second block interval
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal awesomecoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AwesomeCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
